...
async def test_main():
    async with async_playwright() as p:
        ...
        try:
            ...
            # 기대결과: 정상 로그인 및 소셜 탭 페이지로 리다이렉트
            is_social_page = 'social.wanted.co.kr' in final_url
            assert is_social_page, f"소셜 페이지 리다이렉트 실패. 현재 URL: {final_url}"
            print(f"로그인 성공 - 소셜 페이지로 리다이렉트 확인: {final_url}")

            # 세션 저장
            await context.storage_state(path='work/auth_state.json')
            print("세션 저장 완료: work/auth_state.json")

            await page.screenshot(path='screenshots/test_LOGIN_007_success.png')
            print("AUTOMATION_SUCCESS")
            return True

        except Exception as e:
            await page.screenshot(path='screenshots/test_LOGIN_007_failed.png')
