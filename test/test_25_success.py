...
async def test_main():
    async with async_playwright() as p:
        ...
        try:
            ...
            print("[INFO] 교육•이벤트 페이지(event.wanted.co.kr) 접속 중...")
            ...
            # 9. 로그인 후 리다이렉트 대기 (교육•이벤트 페이지로 이동 확인)
            await page.wait_for_load_state('load', timeout=30000)
            await page.wait_for_timeout(3000)
            final_url = page.url
            print(f"[OK] 로그인 후 최종 URL: {final_url}")

            # 10. 교육•이벤트 페이지로 리다이렉트 확인
            is_event_page = 'event.wanted.co.kr' in final_url
            assert is_event_page, (
                f"교육•이벤트 페이지로 리다이렉트 되지 않았습니다. 현재 URL: {final_url}"
            )
            print(f"[OK] 교육•이벤트 페이지로 정상 리다이렉트 확인: {final_url}")

            # 11. 로그인 상태 확인 (로그인 버튼이 사라졌는지 확인)
            try:
                login_indicator = page.locator('a[href*="login"], button:has-text("로그인"), a:has-text("회원가입/로그인")')
                count = await login_indicator.count()
                if count == 0:
                    print("[OK] 로그인 버튼 사라짐 - 로그인 상태 확인")
                else:
                    print(f"[INFO] 로그인 관련 요소 {count}개 존재 (일부는 정상)")
            except Exception as e:
                print(f"[INFO] 로그인 상태 추가 확인 중 예외: {e}")
