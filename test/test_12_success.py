...
async def test_main():
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.screenshot(path='screenshots/test_12_step1_scroll.png')

            # ── 2단계: '합격 가능성 높은 포지션' 섹션 확인 ──
            section_text = '합격 가능성 높은 포지션'
            print(f"[INFO] '{section_text}' 섹션 탐색 중...")

            section_found = False
            for exact in (True, False):
                try:
                    el = page.get_by_text(section_text, exact=exact)
                    if await el.count() > 0:
                        await el.first.scroll_into_view_if_needed()
                        await el.first.wait_for(state='visible', timeout=5000)
                        section_found = True
                        print(f"[OK] '{section_text}' 확인됨 (exact={exact})")
                        break
            ...
            if not section_found:
                ...
                section_found = result.get('found', False)
                print(f"[INFO] JS 탐색 결과: {result}")

            await page.screenshot(path='screenshots/test_12_step2_section.png')
            assert section_found, f"'{section_text}' 텍스트를 찾을 수 없습니다"
            print(f"[OK] '{section_text}' 섹션 확인됨")

            # ── 3단계: 섹션 내 포지션 카드 5개 확인 ──
            print(f"[INFO] '{section_text}' 섹션 포지션 카드 탐색 중...")
            ...
            await page.screenshot(path='screenshots/test_12_step3_cards.png')

            # ── 4단계: 우측 버튼 클릭 후 스크롤 확인 ──
            print(f"[INFO] '{section_text}' 섹션 우측 버튼 탐색 중...")
            ...
            if right_btn_result.get('clicked'):
                ...
                if alt_btn_result.get('clicked'):
                    ...
                    print(f"[WARN] 우측 버튼을 찾지 못함 - '{section_text}' 섹션과 카드는 확인됨")

            await page.screenshot(path='screenshots/test_12_step4_after_right_btn.png')
