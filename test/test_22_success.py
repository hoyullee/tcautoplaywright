...
async def test_main():
    async with async_playwright() as p:
        ...
        try:
            ...
            # GNB에서 소셜 링크 찾기 (social.wanted.co.kr 링크)
            print("🔍 GNB 소셜 항목 탐색 중...")
            social_link = page.locator('a[href*="social.wanted.co.kr"]').first
            await social_link.wait_for(timeout=15000)
            print("✅ 소셜 링크 발견")

            # 소셜 링크가 새 탭으로 열리는지 확인 후 클릭
            href = await social_link.get_attribute('href')
            target_attr = await social_link.get_attribute('target')
            print(f"📌 href: {href}, target: {target_attr}")

            # 새 탭 이벤트를 먼저 대기하면서 클릭
            try:
                print("🆕 새 탭 감지 시도...")
                async with context.expect_page(timeout=8000) as new_page_info:
                    await social_link.click()
                ...
                await page.wait_for_url('*social.wanted.co.kr*', timeout=20000)
            ...
            assert 'social.wanted.co.kr' in current_url, \
                f"소셜 페이지로 이동하지 않았습니다. 현재 URL: {current_url}"
            print("✅ 소셜 페이지 진입 확인")

            await screenshot_page.screenshot(path='screenshots/test_22_success.png')
